---
title: Carbon - Permissions
description: Learn about the permissions system in Carbon
---
...
Permissions are managed within the user module. **An employee's permissions are set according to the default permissions for their employee type when the user is created**. Each employee may then be fine-tuned for individual permissions in the users module. There is also an option to bulk update permissions for employees.
